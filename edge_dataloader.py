...
class BSDS_Loader(data.Dataset):
    ...
    def __init__(self, root='/Dataset/HED-BSDS',
                 split='train', transform=False, threshold=0.3, ablation=False):
        self.root = root
        self.split = split
...
class BIPED_Loader(data.Dataset):
    ...
    def __init__(self, root='/Dataset/BIPEDv2', split='train', threshold=0.5):
        self.root = root
        self.split = split
        self.threshold = threshold * 256
        normalize = transforms.Normalize(mean=[123.68 / 255, 116.779 / 255, 103.939 / 255],
                                         std=[0.229, 0.224, 0.225])
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            ...
            img_file = os.path.join(self.root, self.filelist[index].rstrip())
...
class BSDS_VOCLoader(data.Dataset):
    ...
    def __init__(self, root='/Dataset',
        ...
        self.transform = transforms.Compose([
            ...
            normalize])
        if self.split == 'train':
            self.filelist = os.path.join(self.root, 'HED-BSDS/bsds_pascal_train_pair.lst')
        elif self.split == 'test':
            self.filelist = os.path.join(self.root, 'HED-BSDS/bsds_pascal_test.lst')
        else:
            raise ValueError("Invalid split type!")
